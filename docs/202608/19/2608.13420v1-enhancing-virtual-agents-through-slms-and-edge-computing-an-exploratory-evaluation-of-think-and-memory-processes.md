...
tags: ["query:agent"]
score: 7.0
evidence: 基于认知具身智能体架构与边缘计算的虚拟智能体设计评估
tldr: 虚拟代理需具备持久认知能力，但实现复杂。本文基于CEAA框架，探索用小型语言模型（SLM）在边缘设备上支持“思考”和“记忆”过程。在NVIDIA Jetson Orin NX上集成Qwen2.5模型，构建边缘网关系统，评估路由准确率、记忆读取性能和延迟。实验表明SLM可部分实现CEAA核心流程，为边缘侧高效运行的具身智能代理提供可行方案。
source: arxiv
...
motivation: 实现虚拟代理的认知能力在技术和概念上困难，需探索边缘SLM支撑关键过程。
method: 在Jetson Orin NX上开发边缘网关，用Qwen2.5模型实现CEAA的Think和Memory过程。
result: 实验验证了路由准确率、记忆读取和延迟性能，证明SLM驱动的原型系统可行。
conclusion: SLM可部分实现CEAA，支撑沉浸式虚拟环境中的高效具身代理开发。
---

## 摘要
具身智能虚拟代理预期在复杂的虚拟世界和元宇宙环境中作为持久、自适应且具备上下文意识的实体运行。然而，在此类环境中实现具备认知能力的代理在概念和技术上均具有挑战性。在一系列蓝图和开发方法中，认知具身代理架构（CEAA）已被开发为一种面向实现的框架，用于构建感知、记忆、推理、规划和具身行动等组件。考虑到边缘计算和生成式AI语言模型的最新进展，本文探讨了使用小语言模型（SLM）来支持选定CEAA组件的边缘运行，重点关注“思考”和“记忆”作为交互式虚拟世界中虚拟代理认知编排与持久性的核心过程。本文开发了一个基于边缘的虚拟代理网关系统，并使用不同规模的Qwen2.5模型在NVIDIA Jetson Orin NX上进行了评估，探索了该系统处理服务请求和进行记忆驱动对话的能力。一系列模拟实验评估了路由准确性、记忆读取性能和延迟，展示了一个由SLM驱动的原型代理系统，该系统部分实现了选定的CEAA过程，以支持具身代理的开发，使其认知“大脑”能够高效且具有上下文意识地运行，从而在沉浸式虚拟世界中提供交互体验。
